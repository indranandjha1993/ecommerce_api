...
const Alert: React.FC<AlertProps> = ({
  ...
  return (
    ...
    >
      {alertIcon && (
        <div className={`flex-shrink-0 mr-3 ${variantStyles[variant].icon}`}>
          {alertIcon}
        </div>
      )}
      ...
      <div className="flex-1 min-w-0">
        {title && (
          <h3 className={`text-sm font-medium mb-1 ${variantStyles[variant].title}`}>
            {title}
          </h3>
        )}
        <div className={`text-sm ${variantStyles[variant].content}`}>{children}</div>
      </div>
      
      {onClose && (
        <button
          type="button"
          className={`ml-3 flex-shrink-0 inline-flex h-5 w-5 rounded-full items-center justify-center ${variantStyles[variant].closeButton}`}
          onClick={onClose}
          aria-label="Close"
        >
          <svg
